...
int main (int argc, char *argv[])
{
	...
	std::vector<trajectory_msgs::JointTrajectoryPoint> f_points, l_points, r_points, b_points;
	trajectory_msgs::JointTrajectory f_traj, l_traj, r_traj, b_traj;
	f_traj.joint_names = joint_names;
	l_traj.joint_names = joint_names;
	r_traj.joint_names = joint_names;
	b_traj.joint_names = joint_names;

	std::string f_fname(ros::package::getPath("control_demo")+"/src/testcases/forward.txt");
	std::string l_fname(ros::package::getPath("control_demo")+"/src/testcases/leftward.txt");
	std::string r_fname(ros::package::getPath("control_demo")+"/src/testcases/rightward.txt");
	std::string b_fname(ros::package::getPath("control_demo")+"/src/testcases/backward.txt");

	load_foot_points(f_fname, trans, f_points);
	load_foot_points(l_fname, trans, l_points);
	load_foot_points(r_fname, trans, r_points);
	load_foot_points(b_fname, trans, b_points);

	f_traj.points = f_points;
	l_traj.points = l_points;
	r_traj.points = r_points;
