...
bool zero_position(arobot_trans& trans, std::vector<trajectory_msgs::JointTrajectoryPoint>& points)
{
	time_count += 4.0;
	trajectory_msgs::JointTrajectoryPoint point = get_a_point();
	point.time_from_start = ros::Duration(time_count);
...
bool knee_down(arobot_trans& trans, std::vector<trajectory_msgs::JointTrajectoryPoint>& points)
{
	// Basic infomation
    ...
    // shoulder back
    point.positions[14] = 0.4;
    point.positions[19] = 0.4;
...
int main (int argc, char *argv[])
{
	...
	if(!zero_position(trans, points))
		ROS_INFO_STREAM("Zero position failed");

	if(!knee_down(trans, points))
		ROS_INFO_STREAM("Knee down failed");

	// TODO: iks

	std::string filename(ros::package::getPath("control_demo")+"/src/testcases/test_data.txt");
...
//		trans.getThePose("left_foot", ident_l);
		...
		if(!one_frame_foot(trans, ident_l, ident_r, points, 1.0/ctr_fre))
		{
			ROS_INFO_STREAM("Ik failed");
			// exit(0);
		}
	}
	if(!knee_up(trans, points))
		ROS_INFO_STREAM("Knee up failed");
	...
	pub.publish(traj);
	ROS_INFO_STREAM("l hip max yaw : "<<l_max_hip<<" r hip max yaw: "<<r_max_hip);
	ROS_INFO_STREAM("l hip min yaw : "<<l_min_hip<<" r hip min yaw: "<<r_min_hip);
    /*
    while(ros::ok()){
